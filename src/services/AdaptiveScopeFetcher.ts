...
/**
 * Handles the adaptive fetching strategy for a single scope.
 * It adjusts the time window based on the density of PRs (issueCount) returned by GitHub.
 */
    ...
    try {
      ...
      progress.error =
        err instanceof Error
          ? err
          : new Error(`Unknown error fetching scope: ${this.scope}`);
      progress.done = true;
      this.onProgress({ ...progress });
