...
test.describe('Unauthenticated Home Page', () => {
  ...
  test('should display login prompt and buttons', async ({ page }) => {
    ...
    await expect(page.getByText('Margea', { exact: true }).first()).toBeVisible(
      { timeout: 10000 },
    );
  ...
  test('should navigate to login page when clicking login button in header', async ({
    ...
    await expect(page.getByText(en.loginPage.readOnly)).toBeVisible({
      timeout: 5000,
    });
    await expect(page.getByText(en.loginPage.readWrite)).toBeVisible();
  });
});
