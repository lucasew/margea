...
## 2026-01-24 - Unencrypted Session Cookies Leaking Access Tokens
**Vulnerability:** The session cookie containing the GitHub Access Token was signed (JWS) but not encrypted (JWE). This meant that anyone with access to the cookie string (e.g., via traffic interception, logs, or physical access) could decode the payload and retrieve the sensitive access token, allowing them to impersonate the user on GitHub.
**Learning:** JWS only provides integrity (tamper-proofing), not confidentiality. For sensitive data like access tokens, using `HttpOnly` cookies is not enough defense-in-depth. We must ensure that the data itself is unreadable without the server's secret key.
**Prevention:** Always use JWE (JSON Web Encryption) with strong algorithms (like A256GCM) for storing sensitive data in cookies or tokens. Ensure the encryption key is of the correct length and derived securely from the secret.
