...
## 2026-01-18 - Unencrypted Session Cookies (Information Disclosure)
**Vulnerability:** The session cookie containing the sensitive GitHub access token was stored using a signed JWT (JWS). While the signature prevented tampering, the token's payload (including the access token) was encoded but not encrypted, making it readable to anyone who could access the cookie (e.g., via browser dev tools or XSS).
**Learning:** Sensitive credentials like OAuth access tokens must be treated with the highest level of confidentiality. Relying solely on `HttpOnly` flags is insufficient defense-in-depth. If a cookie is leaked, its contents should remain opaque to the attacker.
**Prevention:** Encrypt sensitive session cookies using JWE (JSON Web Encryption) instead of just signing them. Use robust encryption algorithms like `A256GCM` and ensure the encryption key is securely derived from a server-side secret using a standard key derivation function (like SHA-256).
