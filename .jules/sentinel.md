...
## 2026-01-18 - Sensitive Session Token Leakage via Signed-Only JWT
**Vulnerability:** The session cookie containing the GitHub access token was stored as a signed JWT (JWS) using `HS256`. While signed, the payload is not encrypted and can be easily decoded (Base64Url) by anyone with access to the cookie, leaking the sensitive `github_token`.
**Learning:** Signing only guarantees integrity, not confidentiality. Sensitive data stored in client-side storage (like cookies) must be encrypted to prevent exposure.
**Prevention:** Use JWE (JSON Web Encryption) with `EncryptJWT` and strong encryption algorithms (e.g., `A256GCM`) for any token or cookie containing sensitive secrets. Derive encryption keys securely (e.g., using SHA-256) to meet algorithm requirements.
