...
## 2026-01-24 - Unencrypted Session Tokens
**Vulnerability:** The session cookie containing the GitHub access token was signed (`SignJWT`) but not encrypted. This meant that the access token was base64-encoded and visible to anyone who could access the cookie (e.g., via XSS or network interception if TLS failed), leading to potential information leakage and account compromise.
**Learning:** Signing ensures integrity but not confidentiality. Sensitive data like access tokens must always be encrypted (`EncryptJWT`) when stored on the client side, even in HttpOnly cookies.
**Prevention:** Use `EncryptJWT` with a strong encryption algorithm (e.g., `A256GCM`) for any JWT that contains sensitive secrets. Derive encryption keys securely from the master secret using a KDF (like SHA-256) to satisfy algorithm key length requirements.
