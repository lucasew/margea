...
## 2025-02-12 - Exposed Sensitive Token in Client-Side Requests
**Vulnerability:** The application was fetching the sensitive GitHub access token from the server via `/api/auth/token` and using it to make direct requests to the GitHub GraphQL API from the browser. This pattern exposed the token to XSS attacks and leaked it in the browser's network logs and potentially memory, while also causing CORS issues for users.
**Learning:** Sensitive credentials like OAuth access tokens should never be exposed to the client-side JavaScript environment if possible. Even if HTTPS is used, tokens available to the client can be exfiltrated via XSS. Additionally, client-side requests to third-party APIs are subject to CORS restrictions which can cause reliability issues.
**Prevention:** Use a Backend-for-Frontend (BFF) pattern where the client makes requests to a proxy endpoint on the same origin (e.g., `/api/graphql`). The proxy endpoint handles authentication using an HTTP-only (or at least secure) cookie and injects the access token into the upstream request server-side, keeping the token hidden from the client.
