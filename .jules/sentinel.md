...
## 2026-01-21 - Sensitive Session Data in Signed Cookies
**Vulnerability:** The `session` cookie contained the GitHub access token (`github_token`) and was signed (JWS) but not encrypted (JWE). This meant that anyone with access to the cookie could read the access token.
**Learning:** Signing only guarantees integrity, not confidentiality. Sensitive data like access tokens must always be encrypted when stored on the client side.
**Prevention:** Use JWE (`EncryptJWT`) instead of JWS (`SignJWT`) for any cookie or token that contains secrets. Ensure the encryption key is of sufficient length (e.g., 256 bits for A256GCM).
