...
**- Pattern:** Massive `package-lock.json` diffs (thousands of lines) without corresponding `package.json` changes, often due to platform-specific optional dependencies, environment differences, or isolated transitive dependency bumps.
**- Justification:** Creates noise in PRs, obscures actual dependency updates, and can cause CI instability if dependencies are incorrectly platform-locked. Bumping individual transitive dependencies should be managed via bulk updates or alongside top-level dependency updates.
